...
def main():
    with open(f'day07/input_1.txt') as f:
        puzzle_input = f.read()
    ...
    for test_sum, calibration_values in input_dict.items():
        ...
        operation_permutations = itertools.product('*+', repeat=number_of_operations)
        ...
        for permutation in operation_permutations:
            ...
            for i, symbol in enumerate(permutation):
                result = eval(f"{result} {symbol} {calibration_values[i+1]}")
