...
def walk_trail(step: HikingStep, grid: list[list[str]], visits: set):
    if (step.x, step.y) in visits:
        # If it leads to a path we've already visisted from the given
        # trailhead, we're done searching but won't increase the score.
        return 0

    # Mark the spot as visisted, from the given trailhead 
    visits.add((step.x, step.y))
    ...
    for dy, dx in DIRECTIONS_YX:
        ...
        if height == step.height + 1:
            ...
            score += walk_trail(next_step, grid, visits)
...
def main():
    ...
    for trail_head in trail_heads:
        visits = set()
        scores.append(walk_trail(step=trail_head, grid=grid, visits=visits))
